...
| `ExtendedClassMustBeAbstractOrReferencedRule` | `new ExtendedClassMustBeAbstractOrReferencedRule(layer: 'Source')` | Classes another scanned class extends are declared `abstract` unless they are also referenced as a dependency (instantiated, type-hinted, `::class`, a class-name string, ...). Supports `--fix` by adding the `abstract` modifier. |

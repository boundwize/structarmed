...
| `ExtendedClassMustBeAbstractOrInstantiatedRule` | `new ExtendedClassMustBeAbstractOrInstantiatedRule(layer: 'Source')` | Classes another scanned class extends are declared `abstract` unless they are also instantiated (`new X`, a `new self`/`new static`/`new parent` resolving to them, a constant class expression such as `new (X::class)` or `new ('App\X')`, or a chained `(new ReflectionClass(X::class))->newInstance*()`), or held by a typed property (`private X $x;`, including promoted constructor parameters) — a typed property holds an instance, typically built by a container or factory outside the scanned code. Parameter and return type hints, `instanceof`, and `::class` keep working on an abstract class, so they do not count. Other runtime-fed construction (`new $class` from a parameter, `unserialize()`) is outside the scanned-code boundary — exclude such factories' targets with rule-scoped `skip()` or `skipRule()`. Supports `--fix` by adding the `abstract` modifier. |
